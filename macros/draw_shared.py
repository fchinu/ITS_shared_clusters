"""Script to draw shared clusters analysis results"""
...
def create_single_histogram(labels, title, color):
    ...
    for barplot, count in zip(bars, counts):
        plt.text(
            barplot.get_x() + barplot.get_width() / 2,
            barplot.get_height() + 0.1,
            str(count),
            ha="center",
            va="bottom",
        )
...
def classify_and_create_data(df_shared):
    ...
    return (
        df[~df["isGoodMother"]],
        df[(df["isGoodMother"]) & (~df["same_mc_track_id"])],
        df[df["same_mc_track_id"]],
    )  # fake_data, good_data


def create_2d_histogram_classified(
    fake_data, good_data, doubly_reco_data
):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches
    """Create 2D histogram showing fake vs good particles"""
    # Combine all data to get consistent label ordering
    all_labels = (
        list(fake_data["label"])
        + list(good_data["label"])
        + list(doubly_reco_data["label"])
    )
    ...
    doubly_reco_label_indices = [
        label_to_index[label] for label in doubly_reco_data["label"]
    ]
...
def draw_shared_clusters(
    input_file, output_file, all_tracks=False
):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches
    """
    Main function to draw shared clusters analysis results

    Parameters:
    - input_file: Path to the input Parquet file
    - output_file: Path to the output PDF file
    - all_tracks: Boolean flag to include all tracks or only tracks with shared clusters
    """
    ...
    doubly_reco_labels = (
        list(doubly_reco_data["label"]) if len(doubly_reco_data) > 0 else []
    )
    ...
    with PdfPages(output_file) as pdf:
        ...
        all_sorted_items = sorted(
            all_label_counts.items(), key=lambda x: x[1], reverse=True
        )
        ...
        doubly_reco_counts = [
            doubly_reco_counts_dict.get(label, 0) for label in all_plot_labels
        ]
        ...
        plt.bar(
            ...
            color="red",
            edgecolor="black",
            label=f"Fake (Total: {sum(fake_counts)})",
        )
        plt.bar(
            x_pos,
            doubly_reco_counts,
            ...
            color="green",
            edgecolor="black",
            label=f"Doubly Reco (Total: {sum(doubly_reco_counts)})",
            bottom=fake_counts,
        )
        plt.bar(
            x_pos,
            ...
            color="blue",
            edgecolor="black",
            label=f"Good (Total: {sum(good_counts)})",
            bottom=fake_counts + np.array(doubly_reco_counts),
        )
        ...
        for i, (fake_count, good_count, doubly_reco_count) in enumerate(
            zip(fake_counts, good_counts, doubly_reco_counts)
        ):
            ...
            if total > 0:
                ...
                if doubly_reco_count > 0:
                    plt.text(
                        i,
                        fake_count + doubly_reco_count / 2,
                        str(doubly_reco_count),
                        ha="center",
                        va="center",
                        color="white",
                        fontweight="bold",
                ...
                if good_count > 0:
                    plt.text(
                        i,
                        fake_count + doubly_reco_count + good_count / 2,
                        str(good_count),
                        ha="center",
                        va="center",
                        color="white",
        ...
        if fake_labels:
            create_single_histogram(
                fake_labels, 'Shared Clusters: "Fake" Tracks Only', "red"
            )
            pdf.savefig()
            plt.close()
        ...
        if doubly_reco_labels:
            create_single_histogram(
                doubly_reco_labels, "Shared Clusters: Doubly Reco Tracks Only", "green"
            )
            pdf.savefig()
            plt.close()
        ...
        if good_labels:
            create_single_histogram(
                good_labels, 'Shared Clusters: "Good" Tracks Only', "blue"
            )
            pdf.savefig()
            plt.close()
        ...
        # Count per layer
        fake_counts = (
            df_shared.query("not isGoodMother")["firstSharedLayer"]
            .value_counts()
            .reindex(x_pos, fill_value=0)
        )
        doubly_reco_counts = (
            df_shared.query("same_mc_track_id")["firstSharedLayer"]
            .value_counts()
            .reindex(x_pos, fill_value=0)
        )
        good_counts = (
            df_shared.query("isGoodMother and not same_mc_track_id")["firstSharedLayer"]
            .value_counts()
            .reindex(x_pos, fill_value=0)
        )
        ...
        for i in x_pos:
            ...
            if doubly_reco_counts[i] > 0:
                plt.text(
                    i,
                    fake_counts[i] + doubly_reco_counts[i] / 2,
                    str(doubly_reco_counts[i]),
                    ha="center",
                    va="center",
                    color="black",
                    fontsize=10,
            ...
            if good_counts[i] > 0:
                plt.text(
                    i,
                    fake_counts[i] + doubly_reco_counts[i] + good_counts[i] / 2,
                    str(good_counts[i]),
                    ha="center",
                    va="center",
                    color="black",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input",
        type=str,
        default="CheckTracksCA.parquet",
        help="Input CheckTracksCA parquet file",
    )
    parser.add_argument(
        "output", type=str, default="shared_clusters.pdf", help="Output file"
    )
    parser.add_argument(
        "--all", action="store_true", help="Output all tracks, not only shared"
    )
